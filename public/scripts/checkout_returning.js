$(document).ready(function(){
	...
	/*--- Address Book Logic ---*/
	$('#use_ship_address').live('click',function(event){
		event.preventDefault();
		$('#ship_address_list').append('<div class="processing ch_processing"><div>Please wait...</div></div>');
		$.post(
			'/checkout/use-ship-address/',
			$('#ship_address_list input').serialize(),
			function(data){
				$('.processing').remove();
				$('#current_ship_address').html(data);
				$('#ship_address_list').slideUp(300);
				$('#current_ship_address').slideDown(300);
				calculate_shippment();
			}
		);
	});
	
	$('#use_bill_address').live('click',function(event){
		event.preventDefault();
		$('#bill_address_list').append('<div class="processing ch_processing"><div>Please wait...</div></div>');
		$.post(
			'/checkout/use-bill-address/',
			$('#bill_address_list input').serialize(),
			function(data){
				$('.processing').remove();
				$('#current_bill_address').html(data);
				$('#bill_address_list').slideUp(300);
				$('#current_bill_address').slideDown(300);
			}
		);
	});
	...
	/* --- Chose Shipping/Billing Address --- */
	$('#change_ship_address').live('click',function(event){
		event.preventDefault();
		$('#current_ship_address').slideUp(300);
		$('#ship_address_list').slideDown(300);
	});
	
	$('#change_bill_address').live('click',function(event){
		event.preventDefault();
		$('#current_bill_address').slideUp(300);
		$('#bill_address_list').slideDown(300);
	});
	...
	var checkoutValid = $("#billing_form").validate({
		...
		messages: {
			...
			bill_state: {required:'State is required required field'},
			...
			ship_state: {required:'State is required required field'},
	...
	$('.ship_type_radio').change(function(){
		if ( $('.ship_type_radio:checked').val() != 'add_to_stack' )
		{
			$('#stack_info_div').hide();
			$('#ccard_div').show();
			$('#paypal_block').show();
		...
		else if ( $('#stack_info_div').length > 0 )
		{
			$('#ccard_div').hide();
			$('#stack_info_div').show();
			$('#validate_cc').val(0);
		...
		{
			$('#paypal_block').hide();
			$('#paypal_radio').attr('checked','');
			$('#cc_radio').attr('checked','checked');
		}
	});
	...
	$('#paypal_img').click(function(){
		$('input[name="payment_option"]').removeAttr('checked');
		$('#paypal_radio').attr('checked','checked');
		$('#checkout_submit').trigger('click');
	});
	...
	$('#checkout_submit').click(function(event){
		...
		if ( $('.ship_cost').length < 1 && $('.ship_cost_vintage').length < 1 && !$('#add_to_stack').is(':checked') ) {
...
function set_grand_total()
{
	...
	if ( $('#add_to_stack').is(':checked') ){
		...
		$('#span_ship').text('');
		$('#ship_desc').html('<span class="tRed"><strong>Added to Your Stack</strong></span>');
		$('#ship_reduced').text('');
		
		$('.ship_data').hide();
		$('.ship_vintage_data').hide();
		$('#vintage_ship_hold').hide();
		$('.store_credit').hide();
...
function shipCheck(){
	...
	if ( $('#add_to_stack').is(':checked') ){
		...
		if( typeof(ship_cost) != 'undefined' ){
			set_grand_total();
			if ( $('#ship_country').val() == 'US' ){
				$('#ship_desc').html( '<span class="small tRed"><strong>FREE Shipping on orders $150+</strong></span><a class="modal_text_act" href="/shipoptions/ #free_shipping_sec">Details</a>');
			}
			else{
