...
function set_grand_total(ship_cost) {
	...
	// no paypal if total amount = $0
	if (grand_total_value == 0) $('#paypal_block').hide();
}
...
function checkTax() {		
	...
	var grand_total = order_subtotal + tax + shipping;
	if ( grand_total < credit_total ) {
		used_credit = grand_total;
		grand_total = 0;
	} else {
		grand_total -= credit_total;
		used_credit = credit_total;
	}
	...
	$('#span_credit').html(number_format(used_credit, 2, '.', ''));
	$('#span_grand').text( '$'+grand_total.toFixed(2) );
}
			...
			$.each(data, function(key, val){
				if (key=='options') {
					...
					if ( val ) {
						...
						if ( ship_state == 'MN' ){
							var tax = $('#order_subtotal').val()*1 * 6.875/100;
