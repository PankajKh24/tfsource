$(document).ready(function() {
	...
	$('#catDropdown > ul > li > ul').hover(function() {
		$(this).slideDown(300);
	});
	...
	$('#catDropdown').hover(
		function(){
			if(hovered == 0){
				...
				setTimeout(function() {
					if ($('#hoverOverlay').length<=0 && $('#catDropdown').is(':hover').length >0) {
						$('#page').append('<div id="hoverOverlay"></div>');
					};
				}, 1500);
			}
		...
		function(){
			setTimeout(function() {
				if($('#catDropdown').not(':hover').length >0){
					$('#hoverOverlay').remove();
					hovered = 0;
				}
			}, 1500);
