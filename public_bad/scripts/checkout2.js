$(document).ready(function(){
	...
	/* - Show/Hide Stack Payment Info - */
	/*$('#add_to_stack').change(function(){
		if( $(this).is(':checked') ){
			$('#ccard_div').hide(200);
			$('#stack_info_div').show(200);
		}else {
			$('#ccard_div').show(200);
			$('#stack_info_div').hide(200);
		}
	})*/
				...
				function(data){
					if ( data == 'yes') {
						...
						if( $('#paypal_radio').is(':checked') )
						{
							...
							if ( $('#same_checkbox').is(':checked') ) 
								copyToBill();
							$('#billing_form').attr('action','/checkout2/paypal/');
			...
			$.each(data, function(key, val){
				if (key=='options') {
					...
					if (val == 0)
						...
						$('#span_ship').html( '<span class="span_ship_value">$'+val.toFixed(2)+'</span>' );
				};
			});
			shipCheck();
		},
