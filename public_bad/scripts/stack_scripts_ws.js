					...
					if($("#ship_continue").val() == 'Back to Items')
					{
						$.post('/customer/shipcalculator-new/', $("#form1").serialize(),
							function(data){
								...
								{
									$("#submit_order").show();
									$("#total_div").html('<label class="inp_label w90">Total:</label>' + '<span class="inp_label">$' + (cost*1 + $("#order_subtotal").val()*1).toFixed(2) +'</span>');
								}
								else
								{
									$("#submit_order").hide();
									$("#total_div").html('<label class="inp_label w90">Total:</label>' + '<span class="inp_label">$' + ($("#order_subtotal").val()*1).toFixed(2) + '</span>');
								}
						}, "json");
					}	
				*/					
	...
	*/
	...
	var form1_validate = $("#form1").validate({
		...
		rules: {
			...
			shipment_carrier:{required:true}
		},
		messages: {
			...
			shipment_carrier: "Choose Carrier"
		}//,
